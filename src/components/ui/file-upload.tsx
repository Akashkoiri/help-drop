...
export const FileUpload = ({
  ...
  const handleRemoveFile = (idxToRemove: number, e: React.MouseEvent) => {
    e.stopPropagation();
    const updatedFiles = files.filter((_, idx) => idx !== idxToRemove);
    if (value === undefined) setInternalFiles(updatedFiles);
    if (onChange) {
      onChange(updatedFiles);
    }
              ...
              files.map((file, idx) => (
                ...
                >
                  <button
                    ...
                    className="absolute top-2 right-2 z-50 rounded-full p-1 text-neutral-500 hover:bg-neutral-100 hover:text-red-500 dark:text-neutral-400 dark:hover:bg-neutral-800 dark:hover:text-red-400 transition-colors"
                  >
